                ...
                <div class="panel-body">
                    ...
                    <table id="post-table" class="table table-striped list-table">
                        ...
                        <c:forEach items="${page.content}" var="comment">
                            <tr>
                                <td><strong><img class="avatar" src='../../resource/img/avatar.png' width="32"
                                                 height="32"/>
                                ...
                                <td>
                                    <div>提交于<a href="${g.domain}/posts/${comment.postid}/#comment-${comment.id}"
                                               target="_blank">
                                        <fmt:formatDate value="${comment.createTime}" pattern="yyyy-MM-dd ahh:mm"/></a>
                                        <c:if test="${comment.pid!=null}">| 回复给
                                            <a href="${g.domain}/posts/${comment.postid}/#comment-${comment.pid}"
                                               target="_blank">${comment.pcreator}</a>
                                        </c:if>
                                    </div>
                                    ...
                                    <div class="row-action">
                                        <c:if test="${type=='all'}">
                                            <span><a href="#"
                                                     onclick="zblog.comment.approve('${comment.id}','${comment.status=='wait'?'approve':'wait'}');">${comment.status=='wait'?'批准':'驳回'}</a>&nbsp;|&nbsp;</span>
                                            <span><a href="#"
                                                     onclick="zblog.comment.approve('${comment.id}','reject');">垃圾评论</a>&nbsp;|&nbsp;</span>
                                            <span><a href="#" onclick="zblog.comment.approve('${comment.id}','trash');">移动到回收站</a></span>
                                        </c:if>
                                        <c:if test="${type=='wait'}">
                                            <span><a href="#"
                                                     onclick="zblog.comment.approve('${comment.id}','approve');">批准</a>&nbsp;|&nbsp;</span>
                                ...
                                <td><a href="${g.domain}/posts/${comment.postid}"
                                       target="_blank">${comment.title}</a><br/></td>
                                <td class="center"><span class="glyphicon glyphicon-trash pointer"
                                                         onclick="zblog.comment.remove('${comment.id}')"></span></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
